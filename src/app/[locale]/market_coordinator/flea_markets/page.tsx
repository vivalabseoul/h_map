...
import { getFleaMarketsByCreator, deleteFleaMarket } from '@/lib/database';
import { useAuth } from '@/context/AuthContext';
import type { FleaMarket } from '@/types';
import { Trash2, Pencil } from 'lucide-react';

export default function CoordinatorFleaMarketsPage() {
  const { user } = useAuth();
              ...
              <tr key={m.id} style={{ borderBottom: '1px solid var(--color-border)' }}>
                ...
                <td style={{ padding: 'var(--space-3)' }}>
                  <div style={{ display: 'flex', gap: 'var(--space-2)' }}>
                    <Link href={`/market_coordinator/flea_markets/${m.id}/edit`} style={{ color: 'var(--color-primary)', border: 'none', background: 'none', cursor: 'pointer', padding: '4px' }}>
                      <Pencil size={18} />
                    </Link>
