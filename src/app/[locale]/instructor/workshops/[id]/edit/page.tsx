    ...
    if (user && params.id) {
      Promise.all([getWorkshopsByOwner(user.id), getWorkshops()]).then(([ownedData, allData]) => {
        ...
        if (target) {
          ...
          setIsPrivate(target.isPrivate || false);
        }
    ...
    <div>
      ...
      <div className="card" style={{ maxWidth: '800px' }}>
        <form onSubmit={handleSubmit}>
          ...
          {/* Visibility Toggle */}
          <div className="form-group" style={{ marginBottom: 'var(--space-6)', padding: 'var(--space-3)', background: 'var(--color-bg-alt)', borderRadius: 'var(--radius-md)' }}>
            <label className="form-label" style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', marginBottom: 0, cursor: 'pointer' }}>
              <div>
                <span style={{ fontWeight: 600, display: 'block', marginBottom: '4px' }}>비공개 설정 (Private)</span>
                <span style={{ fontSize: '0.85rem', color: 'var(--color-text-secondary)', fontWeight: 400 }}>스튜디오를 나만 볼 수 있도록 숨깁니다.</span>
              </div>
              <input 
                type="checkbox" 
                checked={isPrivate}
