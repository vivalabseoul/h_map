...
// Analytics Data (to be fetched from real source)
const visitTrendsData: { name: string; visits: number; clicks: number }[] = [];

const trafficSourcesData: { name: string; value: number }[] = [];
...
export default function AdminDashboard() {
  ...
  // Analytics state (to be hydrated with real data)
  ...
  const statCards = [
    ...
    { label: '전체 방문수 (GA 연동 전)', value: analyticsStats.totalVisits.toLocaleString(), icon: <Activity size={24} />, color: '#F5A623' },
    { label: '평균 클릭률 (CTR)', value: `${analyticsStats.avgCTR}%`, icon: <MousePointerClick size={24} />, color: '#9013FE' },
  ];
    ...
    <div>
      <div className="page-header">
        <div>
          ...
          <p>Platform overview and traffic analytics</p>
        </div>
      </div>
      ...
      <div style={{ marginTop: 'var(--space-8)', display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(300px, 1fr))', gap: 'var(--space-6)' }}>
        ...
        <div style={{ background: 'var(--color-surface)', padding: 'var(--space-6)', borderRadius: 'var(--radius-lg)', boxShadow: 'var(--shadow-sm)' }}>
          <h2 style={{ fontSize: '1.125rem', marginBottom: 'var(--space-4)', display: 'flex', alignItems: 'center', gap: '8px' }}>
            <TrendingUp size={20} color="var(--color-primary)" /> 최근 7일 방문 및 클릭 추이
          </h2>
